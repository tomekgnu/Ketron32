...
void initialise(struct MD_MIDIFile *m)
{
...
BOOL isEOF(struct MD_MIDIFile *m)
{
  ...
  for (uint8_t i=0; i<m->_trackCount && bEof; i++)
  {
...
// Reset the file to the start of all tracks
{
  ...
  for (uint8_t i=(m->_looping && m->_trackCount>1 ? 1 : 0); i<m->_trackCount; i++)
...
void processEvents(struct MD_MIDIFile *m,uint16_t ticks)
{
  ...
  for (n = 0; (n < 100) && (!doneEvents); n++)
  {
    ...
    for (uint8_t i = 0; i < m->_trackCount; i++) // cycle through all
    {
      BOOL b;
...
// Load the MIDI file into memory ready for processing
{
  ...
  if ((res = f_open(&m->_fd,m->_fileName,FA_READ)) != FR_OK) 
    return(2);
  ...
  if (dat16 & 0x8000) // top bit set is SMTE format
  {
    ...
    switch (framespersecond) 
    {
      ...
      default:   f_close(&m->_fd); return(7);
    }
